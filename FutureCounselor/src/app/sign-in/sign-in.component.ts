...
export class SignInComponent implements OnInit {
  ...
  signIn(): void
  {
      this.auth.SignIn(this.email, this.password).subscribe
          (user => this.user = user,
        error => this.errorMessage = <any>error, () => this.SaveCookies(this.email, this.user));

  }
 

 
  SaveCookies(email: string, User: user): void {
    this._cookieService.put('name', User.firstname);
    this._cookieService.put('email', email);
    this._cookieService.putObject('id', User.id);
